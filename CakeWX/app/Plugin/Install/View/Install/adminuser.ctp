...
?>
<div class="install">
	<h2><?php echo __d('croogo', '第三步：创建管理员账号'); ?></h2>
	<?php
		$this->Form->inputDefaults(array(
			'label' => false,
			'class' => 'span10',
		));
		echo $this->Form->input('TPerson.FMemberId', array(
			'placeholder' => __d('croogo', '账号'),
			'before' => '<span class="add-on"><i class="icon-user"></i></span>',
			'div' => 'input text input-prepend',
		));
		echo $this->Form->input('TPerson.FPassWord', array(
			'placeholder' => __d('croogo', '密码'),
			'type' => 'password',
			'value' => '',
			'before' => '<span class="add-on"><i class="icon-key"></i></span>',
			'div' => 'input password input-prepend',
		));
		echo $this->Form->input('TPerson.FRePassWord', array(
			'placeholder' => __d('croogo', '确认密码'),
			'type' => 'password',
			'value' => '',
			'before' => '<span class="add-on"><i class="icon-key"></i></span>',
			'div' => 'input password input-prepend',
